...
import inne.streamy.rozwiazania.workshops.domain.*;
import inne.streamy.rozwiazania.workshops.mock.HoldingMockGenerator;
import inne.streamy.rozwiazania.workshops.mock.UserMockGenerator;
...
class WorkShop {
    ...
    private static Predicate<User> isWoman = user -> user.getSex().equals(Sex.WOMAN);
    ...
    /**
     * Zwraca nazwy wszystkich holdingów pisane z ?? litery w formie listy.
     */
     ...
     */
    String testStream() {
        return holdings.stream().toString();
    }
...
//                .count();
    ...
    long getAllUserAmount() {
        return holdings.stream()
                ...
                .sum();
        //Stream<List<Cpomanies>> -> Stream<Companies>
    }
...
//                .collect(Collectors.groupingBy(accountType -> accountType, Collectors.counting()))
    ...
    Map<Boolean, Set<String>> getUserBySex() {
        ...
        return getUserStream()
                .filter(isManOrWoman)
                .collect(partitioningBy(isWoman, mapping(User::getLastName, toSet())));
    }
